...
class Tab_Manage {
  ...
  _buildCards() {
    ...
    this._totalCardW = cols * (cw + gap) - gap;
  }
  ...
  _openPopup(char) {
    ...
    const jobHit = scene.add.rectangle(contentX, curY, jobLbl.width, parseInt(fs(14)), 0, 0)
  ...
  _showTooltip(x, y, text) {
    ...
    const fs  = n => scaledFontSize(n, scene.scale);
    const pad = 10;
    const maxW = parseInt(scaledFontSize(200, scene.scale));
