...
class Tab_Squad {
  ...
  _buildGrid(gx, gy, cs) {
    ...
    // 잠수정 내부 텍스트
    const subLbl1 = scene.add.text(subCx, subCy - parseInt(scaledFontSize(7, scene.scale)), '잠수정', {
      fontSize: scaledFontSize(7, scene.scale), fill: '#4a8aaa', fontFamily: FontManager.MONO,
    }).setOrigin(0.5, 0.5);
    const subLbl2 = scene.add.text(subCx, subCy + parseInt(scaledFontSize(7, scene.scale)), 'SUB', {
      fontSize: scaledFontSize(7, scene.scale), fill: '#2a5868', fontFamily: FontManager.MONO,
    }).setOrigin(0.5, 0.5);
    // 잠수정→격자 연결 화살표
    ...
    const arrW   = cs * 0.52;   // 화살표 전체 폭
    const arrH   = cs * 0.44;   // 화살표 머리 높이
    const shaftH = cs * 0.20;   // 화살대 높이
    const shaftW = arrW * 0.44;

    const entGfx = scene.add.graphics();
    entGfx.fillStyle(0x4a1010, 0.85);
    entGfx.lineStyle(2, 0xaa3020, 0.9);
    // 화살표 다각형 (왼쪽을 가리킴 ←)
    const ax = entCx + arrW/2;  // 오른쪽 끝 (꼭지점 반대쪽)
    const tip = entCx - arrW/2; // 화살 끝 (왼쪽)
    const pts = [
      { x: ax,              y: entCy - shaftH/2 },  // 위-오른쪽 몸통
      { x: ax,              y: entCy + shaftH/2 },  // 아래-오른쪽 몸통
      { x: entCx,           y: entCy + shaftH/2 },  // 아래 꺾임
      { x: entCx,           y: entCy + arrH/2   },  // 날개 아래
      { x: tip,             y: entCy            },  // 화살 끝
      { x: entCx,           y: entCy - arrH/2   },  // 날개 위
      { x: entCx,           y: entCy - shaftH/2 },  // 위 꺾임
    ];
    entGfx.fillPoints(pts, true);
    entGfx.strokePoints(pts, true);

    const entLbl = scene.add.text(entCx + arrW*0.05, entCy + arrH/2 + parseInt(scaledFontSize(5,scene.scale)),
      '입  구', {
      fontSize: scaledFontSize(7, scene.scale), fill: '#8a4030', fontFamily: FontManager.MONO,
    }).setOrigin(0.5, 0);
    // 격자→입구 연결선
    const entLine = scene.add.graphics();
    entLine.lineStyle(1, 0x6a2010, 0.5);
    entLine.lineBetween(gx + cs*3, entCy, entCx - arrW/2, entCy);
  ...
  _makeSliderCard(char, x, y, cw, ch) {
    ...
    const icon = scene.add.text(cw/2, ch*0.28,
      JOB_SHORT[char.job] || '???', {
      fontSize:scaledFontSize(9,scene.scale), fill:'#5a7888', fontFamily:FontManager.MONO,
    }).setOrigin(0.5);
    const nameT = scene.add.text(cw/2, ch*0.50, char.name,{
  ...
  // ── 슬라이더 드래그 ───────────────────────────────────────────
  // dragHit 대신 scene.input 레벨로 처리 → 슬라이더 카드 클릭 차단 방지
  _buildSliderDrag() {
    const { scene } = this;
    const aX = this._sliderAreaX;
    const aY = this._sliderAreaY;
